	...
	<ModalTemplate class-list="grid place-items-center p-4" :is-modal-visible="isVisible" :close-func="closeThisModal"
		...
		<div class="bg-white w-[480px] mx-auto mobile:w-full relative p-6 rounded-lg" @click.stop>
			...
			<div class="flex flex-col gap-4 mt-4 mb-6">
				<input1 inp-id="inpRegNewOrgName"
								v-model.trim="organization.name"
								:model-value="organization.name"
								:placeholder="$t('dashboard.namePlaceholder')" :label="$t('dashboard.organizationName')"/>
				<input1 inp-id="inpRegNewOrgSite"
								v-model.trim="organization.website" :model-value="organization.website"
								placeholder="organization.com" :label="$t('dashboard.website')"/>
				<input1 v-model.trim="organization.address" :model-value="organization.address"
								:label="$t('dashboard.address')" :placeholder="$t('dashboard.address')"/>
				<div class="text-h4 text-gray-c-500 text-left font-normal">
					{{ $t('general.email') }}
				</div>
				<input1 v-for="(item, index) in organization.emails"
								:placeholder="$t('general.email')" validationType="mail"
				...
				<ButtonText1 class="group p-2 font-medium w-min flex flex-nowrap items-center"
										 v-if="isAddEmailVisible" @click="addEmail">
			...
			<button-1 class="w-full" :disabled="!isAllDataValid" @click.stop="InviteOrganization">
				{{ $t('general.save') }}
			</button-1>
	...
	methods: {
		...
		async InviteOrganization(){
			if(!this.isAllDataValid)
				return
			await api.organizations.inviteOrganization(this.organization)
				.then(res=>{
					console.log(res)
