  ...
  >
    <div ref="wrapper" class="w-[240px] rounded-lg" >
      <div class="h-6 bg-green-c-200 pt-2">
        <div class="w-8 h-8 rounded-full mx-auto bg-white p-2 overflow-visible">
          <SVGBuildingCondition :class="getSVGColorClass(reportFlags.buildingCondition, marker.reports.buildingCondition)"/>
        </div>
      </div>
      <div ref="content" class="p-4 text-center text-b3" >
        ...
        <p>
          reported on {{GetDateTimeShort(marker.updated_at)}}
        </p>
      </div>
    </div>
  </GMapInfoWindow>
