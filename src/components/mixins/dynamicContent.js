...
export default {
  ...
  methods: {
    ReportAddressFull(report) {
      let address = ''
      if (report.address) address += `${report.address}, `
      if (report.street_number) address += `${report.street_number}, `
      if (report.index) address += `${report.index}, `
      if (report.city) address += `${report.city}`
      let trim = 0
