...
<template>
  <div class="flex flex-col gap-6">
    ...
    <div data-org-logo>
      ...
      <div
        class="
          relative my-2 grid h-[160px] content-center
          justify-center rounded-xl border border-gray-c-300
        "
      >
        <div
          v-if="!logoImgSrc"
        ...
        >
          <div class="font-medium text-gray-c-500">
            Logo
          </div>
        </div>
        <img
...
export default {
  ...
  computed: {
    DescSymbolLimStr() {
      if (!this.modelValue || !this.modelValue.description) return `0/${this.descSymbolLim} ${this.$t('general.symbols')}`
  ...
  watch: {
    ...
    'modelValue.description': function (newVal, oldVal) {
      if (!newVal) return
    ...
    // eslint-disable-next-line func-names
    'modelValue.logo': function (newVal) {
      // console.log(newVal)
