<template>
	<div class="p-6 comp:p-9 overflow-y-auto-custom h-full">
		...
		<div data-search-inputs-group class="my-3 flex flex-col gap-2.5 comp:flex-row comp:gap-3">
			<input-search class="w-full comp:max-w-[400px] shrink grow"
		...
		<div class="mb-6 flex justify-end">
			<Button1 class="w-full comp:w-min min-w-[120px]" @click="getOrganizationChangeLog">
				{{$t("general.search")}}
			</Button1>
		</div>
...
export default {
	...
	methods : {
		...
		updateWorkerSuggestion(){
			...
			if(!this.userOrg || !this.userOrg.participants)
				return;
		...
		getOrganizationChangeLog(){
			...
			if(interval)
				...
				payload["date_max"] = Math.round(Date.now()/1000);
		...
		async toggleRecordVisibility(id){
			await api.changelogs.locationChangelogVisibilityToggle(id)
				.then(res=>{
					let log = this.logs.find(x=>x.id===res.data.id);
					if(log)
						log.hidden=res.data.hidden;
				})
				.catch(err=>{
