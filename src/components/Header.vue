				...
				:class="{	'mobile:left-0' : isMenuVisibleOnMobile,
					...
					<div class="flex mobile:flex-col font-medium mobile:font-semibold text-blue-c-500
						...
						<div v-if="isAuth" class="flex items-center gap-x-6
							...
							<div class="menu-item menu-item-mobile" v-if="getRole!== userRoles.organizationAdmin" id="settings" @click="showSettingModal">
...
<!--									<img src="/src/assets/Organizations/List.svg" class="h-4 ml-1 w-auto">-->
							...
							<MenuItemLink to="/organization/profile" class="w-full hidden mobile:block order-3"  v-if="getRole== userRoles.organizationAdmin">
								<template #image>
									<SVG_Org_List/>
								</template>
								<template #text>
							...
							<MenuItemLink to="/organization/change-history" class="w-full hidden mobile:block order-2"  v-if="getRole== userRoles.organizationAdmin">
								<template #image>
									<SVG_history/>
								</template>
								<template #text>
							...
							<MenuItemLink to="/organization/settings" v-if="getRole== userRoles.organizationAdmin" class="w-full hidden mobile:block order-1">
								<template #image>
									<SVG_settings/>
								</template>
								<template #text>
			...
			<div class="flex flex-nowrap gap-3">
				<div class="h-min font-medium text-gray-c-600" v-if="isAdminButtonsVisible">
					<router-link to="/main">
						<div class="inline-block px-3 py-2 rounded-l-lg border"
							:class="{'border-blue-c-500 bg-blue-c-500 text-white' : !isDashboardPage,
												'border-gray-c-300' : isDashboardPage}">

							<svg class="mobile:inline hidden" width="20" height="20" viewBox="0 0 20 20"
									 :class="{'fill-white' : !isDashboardPage,
					...
					<router-link :to="dashboardPath">
						<div class="px-3 py-2 rounded-r-lg border inline-block"
										:class="{'border-blue-c-500 bg-blue-c-500 text-white' : isDashboardPage,
												'border-gray-c-300' : !isDashboardPage}">

							<svg class="hidden" width="20" height="20" viewBox="0 0 20 20"
									 :class="{'fill-white' : isDashboardPage,
												'fill-gray-c-600' : !isDashboardPage,
          ...
          {{ $t('header.login') }}
				...
				<button-text1 @click="LogOutUser" v-if="isAuth" id="logoutButton" class="h-min my-auto">
          {{ $t('header.logout') }}
				</button-text1>
			</div>
<!--			#region Modals-->
  ...
  methods : {
	...
	computed : {
		...
		isAdminButtonsVisible(){
			return this.isRoleHaveAccess(this.getRole, this.userRoles.organizationAdmin)
		},
		dashboardPath(){
			let path = "/"
			switch (this.getRole){
