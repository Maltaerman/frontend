	  ...
	  <div>
			<keep-alive>
				<ReportsRequestsList v-if="selectedTabItem===`All requests`"
														 ...
														 :page="requestedMarkers.page" :page-max="requestedMarkers.pageMax"
														 @next-page="GetReportsRequest"
														 @add-to-my-list="OnAddToMyList"
				/>
			</keep-alive>
				<MyReportRequestList v-if="selectedTabItem===`My requests`" :my-unreviewed-markers="myList.myUnreviewedMarkers"
  ...
  methods : {
	  setSelectedTab(tabName){
		...
		async GetRequestsPage(payload){
			await api.locations.getReportsRequests(payload).then(res=>{
				if(res.data.length === 0)
					...
					this.requestedMarkers.pageMax = this.requestedMarkers.page;
				this.requestedMarkers.unreviewedMarkers = [...this.requestedMarkers.unreviewedMarkers, ...res.data];
			}).catch(err=>{
		...
		OnRemoveFromMyList(req){
			this.myList.myUnreviewedMarkers = this.myList.myUnreviewedMarkers
				.filter(el=>el.id!==req.id);
			let removedReq = this.requestedMarkers.unreviewedMarkers.find(x=>x.id === req.id);
			if(removedReq)
				removedReq.reported_by=req.reported_by;
		},
		OnAddToMyList(req){
			let addedReq = this.requestedMarkers.unreviewedMarkers.find(x=>x.id === req.id)
			if(addedReq) {
				addedReq.reported_by = req.reported_by;
				this.myList.myUnreviewedMarkers = [addedReq, ...this.myList.myUnreviewedMarkers];
			}
		}
  },
	computed : {
